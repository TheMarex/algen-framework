...
#include "pq/addressable/microbenchmark.h"
...
int main(int argc, char** argv) {
    ...
    const std::string resultfn_prefix = args.get<std::string>("p", "results_addr_pq_"),
                      serializationfn = args.get<std::string>("o", "data_addr_pq.txt");
    ...
    using PQ = pq::addressable::priority_queue<unsigned>;
    ...
    pq::addressable::pairing_heap<unsigned>::register_contenders(contenders);

#if defined(__GNUG__) && !(defined(__APPLE_CC__))
    // These are from GNU libstdc++ policy-based datastructures library
    // Only use if available
    pq::addressable::gnu_pq<unsigned>::register_contenders(contenders);
#endif

    // Register Benchmarks
    common::contender_list<Benchmark> benchmarks;
    pq::addressable::microbenchmark<PQ>::register_benchmarks(benchmarks);
