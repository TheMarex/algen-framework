...
class addressable_pairing_heap_vector
{
public:
    ...
    addressable_pairing_heap_vector() : _top(nullptr), _size(0) {}
    ~addressable_pairing_heap_vector()
    {
        for (auto* r : _roots)
        {
            _free.release(r);
    ...
    void pop()
    {
        ...
        auto end = std::remove_if(_roots.begin(), _roots.end(), [this](elem* root) { return root == _top; });
        _roots.resize(std::distance(_roots.begin(), end));
